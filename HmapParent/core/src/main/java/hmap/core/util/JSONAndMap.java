...
public class JSONAndMap {
    ...
    * */
    public static String jsonToXml(String json, String namespace) throws HmsApiException {
...
//        return map;
    ...
    * map转成xml
    * */
    public static String map2Xml(Map map, String namespace) throws HmsApiException {
...
//            Object rootName = map.keySet().iterator().next();
            ...
            throw new HmsApiException(HmsApiException.ERROR_MAP_TO_XML, "格式错误");
        }
    ...
    * 把返回的xml转化成map
    * */
    public static Map xml2map(String xmlString,HeaderAndLineDTO headerAndLineDTO) throws HmsApiException {
        Document doc = null;
        ...
        try {
            ...
            if (rootElement.elements().size() == 0) {
                ...
                ele2map(tempMap, rootElement,headerAndLineDTO);
                if (rootElement.getNamespace().getText() != null && rootElement.getNamespace().getText() != "") {
                    for (Object obj : rootElement.declaredNamespaces()) {
                        Namespace ns = (Namespace) obj;
            ...
            throw new HmsApiException(HmsApiException.ERROR_XML_TO_MAP, "格式错误");
        }
        return map;
    }
    ...
    private static void ele2map(Map map, Element ele,HeaderAndLineDTO headerAndLineDTO) {
        String[] objList = null;
        if(headerAndLineDTO != null){
//            logger.info("headerAndLineDTO.getObjToArr:"+headerAndLineDTO.getObjToArr());
            if(headerAndLineDTO.getObjToArr() != null && !"".equals(headerAndLineDTO.getObjToArr())){
                objList = headerAndLineDTO.getObjToArr().split(";");
            }
        }
        logger.info("objList:"+objList);
        List<Element> elements = ele.elements();
        logger.info("attachments elements.size():"+elements.size());
        if (elements.size() == 1) {
            logger.info("getName(elements.get(0)11):" + getName(elements.get(0)));
            Boolean flag = true;
            if(objList != null && objList.length > 0){
                for(int i=0;i<objList.length;i++){
                    logger.info("elements1 objList[i]:" + objList[i]);
                    if(objList[i].equals(getName(elements.get(0)))){
                        flag = false;
                        break;
                    }
                }
            }
            logger.info("elements1 flag:"+flag);
            if (flag == false) {
                List<Object> list = new ArrayList<Object>();
                for (Element element : elements) {
                    if (element.elements().size() == 0) {
                        if("".equals(element.getText()) || element.getText() == ""){
                        ...
                        ele2map(tempMap1, element,headerAndLineDTO);
                        list.add(tempMap1);
                    }
                }
                map.put(getName(elements.get(0)), list);
            } else {
                logger.info("elements.get(0).elements().size():"+elements.get(0).elements().size());
                if (elements.get(0).elements().size() == 0) {
                    map.put(getName(elements.get(0)), elements.get(0).getText() == "" ? "" : elements.get(0).getText());
                } else {
                    logger.info("getName(elements.get(0)22):" + getName(elements.get(0)));
                    Map<String, Object> tempMap = new HashMap<String, Object>();
                    ele2map(tempMap, elements.get(0),headerAndLineDTO);
            ...
            for (Element childElement : elements) {
                ...
                if (tempMap.get(string) == null) {
                    ...
                    if(objList != null && objList.length > 0) {
                        for (int i = 0; i < objList.length; i++) {
                            logger.info("elements2 objList[i]:" + objList[i]);
                            if (objList[i].equals(getName(elements2.get(0)))) {
                                flag = false;
                                break;
                            }
                        }
                    }
                    logger.info("elements2 flag:"+flag);
                    if (elements2.size() > 1 || flag ==false) {
                        List<Object> list = new ArrayList<Object>();
                        ...
                        if (elements2.get(0).elements().size() == 0) {
                            ...
                            ele2map(tempMap1, elements2.get(0),headerAndLineDTO);
