...
@Service
public class PLSQLApiServiceImpl implements IApiService {
  @Autowired
  private JDBCSqlSessionFactory hrmsSqlSessionFactory;
  private Logger logger = LoggerFactory.getLogger(PLSQLApiServiceImpl.class);
  ...
  public JSONObject invoke(HeaderAndLineDTO headerAndLineDTO, JSONObject inbound) {
    ...
    try {
      ...
      // 访问数据库
      // 调用oracle package
      Object resp = hrmsSqlSessionFactory.getJdbcTemplateObject().execute(executeSql,
