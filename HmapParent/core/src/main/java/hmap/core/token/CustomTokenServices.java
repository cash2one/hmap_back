    ...
    public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication)
        ...
        if (existingAccessToken != null) {
            ...
            if (config != null) {
                ...
                HmsSystemConfig hmsSystemConfig = hmsSystemConfigService.selectByConfigKey("hmap.singleClient");
                if(hmsSystemConfig != null) {
                    singleClient =hmsSystemConfig.getConfigValue();
                }
            }

            //每次请求token都产生一个新的token，并删除旧的token
            if ((!grantType.equals("client_credentials")) && "N".equals(singleUser) ||
                    (grantType.equals("client_credentials") && "N".equals(singleClient))) {
